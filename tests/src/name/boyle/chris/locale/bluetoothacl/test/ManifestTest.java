...
package name.boyle.chris.locale.bluetoothacl.test;
...
public final class ManifestTest extends AndroidTestCase
{
    ...
    /**
     * Verifies that a plug-in condition Activity is present, enabled, exported, doesn't require permissions, and has a name and
     * icon.
     */
    ...
    /**
     * Verifies that the plug-in exports a single {@link com.twofortyfouram.locale.Intent#ACTION_QUERY_CONDITION}
     * BroadcastReceiver.
     */
    ...
    private static List<ResolveInfo> getPluginActivities(final Context context)
    {
        ...
        for (final ResolveInfo x : context.getPackageManager()
                                          .queryIntentActivities(new Intent(com.twofortyfouram.locale.Intent.ACTION_EDIT_CONDITION), PackageManager.GET_ACTIVITIES))
        {
    ...
    private static List<ResolveInfo> getPluginReceivers(final Context context)
    {
        ...
        for (final ResolveInfo x : context.getPackageManager()
                                          .queryBroadcastReceivers(new Intent(com.twofortyfouram.locale.Intent.ACTION_QUERY_CONDITION), PackageManager.GET_INTENT_FILTERS))
        {
            if (packageName.equals(x.activityInfo.packageName))
            {
                result.add(x);
